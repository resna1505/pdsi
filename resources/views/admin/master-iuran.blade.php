        ...
        <div class="card">
                ...
                <div class="row justify-content-between">
                    ...
                    <div class="col-sm-auto ms-auto">
                        <div class="list-grid-nav hstack gap-1">
                            <button class="btn btn-info addMembers-modal" data-bs-toggle="modal" data-bs-target="#addmemberModal"><i class="ri-add-fill me-1 align-bottom"></i> Add Master</button>
                        </div>
                    </div>
                </div>
                <div class="mt-4">
                    <ul class="nav nav-tabs nav-tabs-custom nav-secondary" role="tablist">
                       <li class="nav-item">
                            <a class="nav-link active" data-bs-toggle="tab" href="#dataMaster" role="tab" aria-selected="false">
                                Master
                            </a>
                        </li>
                    </ul>
            ...
            <div class="card-body p-4">
                <div class="tab-content">
                    <div class="tab-pane active" id="dataMaster" role="tabpanel">
                                            ...
                                            <div class="flex-grow-1 ms-sm-3 mt-2 mt-sm-0">
                                                <div class="position-relative">
                                                    <a href="javascript:void(0);" class="stretched-link">
                                                        <h5>{{ $item->nama_iuran }}</h5>
                                                    </a>
                                                </div>
                                                <p class="text-muted mb-2">Rp. {{ number_format($item->jumlah, 0, ',', '.') }}</p>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0">
            <div class="modal-header p-4 pb-0">
                <h5 class="modal-title" id="createMemberLabel">Add Master</h5>
                <button type="button" class="btn-close" id="createMemberBtn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
                <form method="POST" action="{{ route('masteriuran.store') }}" enctype="multipart/form-data" id="memberlist-form" class="needs-validation" novalidate>
                @csrf
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="title" class="form-label">Nama Iuran</label>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('periode') is-invalid @enderror" data-provider="flatpickr" id="periode" name="periode" value="{{ old('periode') }}">
                        @error('periode')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>                
...
<div class="modal fade" id="editmemberModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0">
            <div class="modal-header p-4 pb-0">
                <h5 class="modal-title" id="editMemberLabel">Edit Master</h5>
                ...
                <form method="POST" action="" enctype="multipart/form-data" id="edit-form" class="needs-validation" novalidate>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3" id="title-wrapper">
                            <label for="edit-title"  class="form-label">Nama Iuran</label>
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="periode" class="form-label">Periode</label>
                            <input type="date" class="form-control" data-provider="flatpickr" id="edit-periode" name="periode">
                        </div>                        
                        <div class="hstack gap-2 justify-content-end">
                            <button type="button" class="btn btn-light" data-bs-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-success">Update Master</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        removeLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                const actionUrl = `/masteriuran/${articleId}`;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        editLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                // Fetch article data
                fetch(`/masteriuran/${articleId}/edit`)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // Set form action
                            editForm.setAttribute('action', `/masteriuran/${articleId}`);
                            
                            // Fill form fields
                            document.getElementById('edit-title').value = article.nama_iuran;
                            document.getElementById('edit-jumlah').value = article.jumlah;
                            document.getElementById('edit-periode').value = article.periode;
...
<script>
    const cleave = new Cleave('#jumlah', {
        numeral: true,
        numeralThousandsGroupStyle: 'thousand',
        rawValueTrimPrefix: true
    });

    // Pastikan nilai yang dikirim tetap 50000 (tanpa titik)
